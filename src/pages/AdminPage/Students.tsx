...
interface Student {
  ...
  Links: string[];
  Image: string;
  Awards: string[]; // Add this line
  exam: string[];   // Add this line
}
...
const Students: React.FC = () => {
  ...
  const [experience, setExperience] = useState<string[]>([]);
  const [links, setLinks] = useState<string[]>([]);
  const [awards, setAwards] = useState<string[]>([]); // Add this line
  const [exam, setExam] = useState<string[]>([]);     // Add this line
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string | null>(null);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Append each item in the Awards array
    awards.forEach((award, index) => {
      formData.append(`Awards[${index}]`, award);
    });

    // Append each item in the Exam array
    exam.forEach((examItem, index) => {
      formData.append(`exam[${index}]`, examItem);
    });
  ...
  const handleEdit = (student: Student) => {
    ...
    setExperience(student.Experience);
    setLinks(student.Links);
    setAwards(student.Awards); // Add this line
    setExam(student.exam);     // Add this line
  ...
  const closeModal = () => {
    ...
    setExperience([]);
    setLinks([]);
    setAwards([]); // Add this line
    setExam([]);   // Add this line
    setSelectedFile(null);
    setImagePreview(null);
  };
          ...
          <table className="w-full text-sm text-left text-gray-500">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3">Experience</th>
                <th className="px-6 py-3">Links</th>
                <th className="px-6 py-3">Awards</th> {/* Add this line */}
                <th className="px-6 py-3">Exam</th>   {/* Add this line */}
                <th className="px-6 py-3">Image</th>
                <th className="px-6 py-3">Actions</th>
              </tr>
            </thead>
            <tbody>
              {students.map((student) => (
                <tr key={student._id} className="border-b">
                  ...
                  <td className="px-6 py-4">{student.Experience.join(", ")}</td>
                  <td className="px-6 py-4">{student.Links.join(", ")}</td>
                  <td className="px-6 py-4">{student.Awards.join(", ")}</td> {/* Add this line */}
                  <td className="px-6 py-4">{student.exam.join(", ")}</td>   {/* Add this line */}
                  <td className="px-6 py-4">
                    <img
                      src={student.Image}
                      alt={student.Name}
